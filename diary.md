...
# ~~БД~~:  неактуально, поменял на PostgreSQL 
...
Поменял схему подключения, берем строку подключения из переменной DBCONN полностью, а не только пароль.
База `backend` по-старому.

## Запуск в Docker-контейнере
Чтобы прерывалось по Ctrl+C, нужно указывать `ENTRYPOINT ["run.sh"]`, а не `RUN run.sh`, и иметь в Dckerfile `STOPSIGNAL SIGINT`.

## Переехал на PostgreSQL

Почему: Контейнер гораздо меньше: `postgres` весит 376 МБ против 1.78 ГБ у `mcr.microsoft.com/mssql/server`. Решил 
завернуть свое приложение в один контейнер поверх postgres, а не отдельный с compose, чтобы потом попробовать запустить контейнер на Heroku. Появились 
проблемы с кодом Entity Framework: по-другому указываются дефолты для столбцов, локаль, по другом работает 
`IDENTITY`. Пришлось вставлять ifы, и в SeedDB вручную переустаналивать начальное значение после генерации 
данных. 

## Переработана схема подключения к БД
Используется URL из переменной DATABASE_URL, как в Heroku. По умолчанию
postgres://postgres:InsideContainer2022@localhost:54321. 
Выкинул SeedDB, инициализация БД и заполнение нач данными происходит
во время старта приложения.
Так как сервер БД в контейнере, не страшно, если мы засветим пароль. 
Изменил скрипт запуска БД на использовани дефолтного пароля.

## Изменения в WebAPI
Исключены пустые ссылки на из клиента на заказы и обратно, включены только id. Потребовалось
поменять иерархию классов.

Sun Apr 17 13:45:35 +09 2022